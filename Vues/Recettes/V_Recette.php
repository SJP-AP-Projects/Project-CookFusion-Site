      ...
      <div
        ...
        <div
          ...
          <table class="min-w-full border-collapse bg-white">
            <thead>
              <tr>
                <th class="px-6 py-3 text-left text-sm font-semibold border-r-2 border-solid">Date</th>
                <th class="px-6 py-3 text-left text-sm font-semibold border-r-2 border-solid">Nom</th>
                <th class="px-6 py-3 text-left text-sm font-semibold border-r-2 border-solid">Boutons</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
              <?php foreach ($lesSessions as $session) : ?>
                <tr>
                  <td class="px-6 py-4 border-r-2 border-solid border-t-2">
                    <?php echo $session->getdate(); ?>
                  </td>
                  <td class="px-6 py-4 border-r-2 border-solid border-t-2">
                    <?php echo $session->getLibelle(); ?>
                  </td>
                  <td class="border-solid border-t-2">
                    <a href="index.php?controleur=Sessions&action=Session&id=<?php echo $session->getId(); ?>">
                      <button class="bg-amber-100 text-black font-bold py-2 px-4 rounded-full">
                        Voir la session
                      </button>
                    </a>
                  </td>
